...
angular.module('common.services.treeview.ivh-treeview-mgr', [])
  .factory('ivhTreeviewMgr', ['ivhTreeviewOptions', 'ivhTreeviewBfs', function(ivhTreeviewOptions, ivhTreeviewBfs) {
    ...
    var makeDeselected = function(node) {
      node[options.selectedAttribute] = false;
      node[options.indeterminateAttribute] = false;
    };

    var makeSelected = function(node) {
      node[options.selectedAttribute] = true;
      node[options.indeterminateAttribute] = false;
    };
    ...
    /**
     * CUSTOM LOGIC CREATED BY STG
     */

    exports.validateSelectionChange = function(tree, node, opts, multiSelect) {
      //if multi select is not enabled then uncheck all nodes except for the newly selected node
      if (multiSelect === false || multiSelect === 'false') {
        //traverse the entire tree and deselct all nodes except for the newly selected node
        ivhTreeviewBfs(tree, opts, function(n, p){
          if (node[opts.idAttribute] !== n[opts.idAttribute] && n[opts.selectedAttribute] === true) {
            exports.deselect(tree, n, opts);
          }
        });
      } else {
        //otherwise ensure that no child nodes are selected if the parent is selected
        ivhTreeviewBfs(node, ivhTreeviewOptions, function(n, p){
          if (node[opts.idAttribute] !== n[opts.idAttribute] && n[opts.selectedAttribute] === true) {
            exports.deselect(tree, n, opts);
          }
        });
