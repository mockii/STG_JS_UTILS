...
angular.module('common.directives.treeview.ivh-treeview', []).directive('ivhTreeview', ['ivhTreeviewMgr', function(ivhTreeviewMgr) {
  'use strict';
  return {
    restrict: 'A',
    transclude: true,
    ...
    controller: ['$scope', '$element', '$attrs', '$transclude', 'ivhTreeviewOptions', 'filterFilter', function($scope, $element, $attrs, $transclude, ivhTreeviewOptions, filterFilter) {
      ...
      /**
       * Returns `true` if we should use highlight selected node, false otherwise
       *
       * Note: highlight selected node will be by useCheckboxes(multi selected) to false
       *
       * @return {Boolean} Whether or not to use highlight selected
       */
      trvw.useHighlightSelected = function() {
        return !localOpts.useCheckboxes;
      };
      ...
      /**
       * Returns `true` if `node` is blocked, indicating that a parent node is already selected.
       * This is intended to be used in conjuction with checkboxes.
       *
       * @param {Object} node The node to check
       * @return {Boolean} `true` if `node` is blocked for selection
       */
      trvw.isBlocked = function(node) {
        return node[localOpts.blockedAttribute];
      };
      ...
      trvw.isLeaf = function(node) {
        return (node.hasOwnProperty(localOpts.leafAttribute)) ? node[localOpts.leafAttribute] : trvw.children(node).length === 0;
      };
